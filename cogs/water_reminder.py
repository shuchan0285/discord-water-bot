...
class SleepButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label="我要睡覺了", style=discord.ButtonStyle.blurple, custom_id="sleep_btn")
...
class WaterButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="喝了！", style=discord.ButtonStyle.success, custom_id="drink_water_btn")
    async def drink_button_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if drawn_event:
            lines.append(f"> **突發機緣**：{drawn_event['exp']:+d} EXP ({drawn_event['text']})")
        ...
        if old_total == 0:
            ...
            if role_id:
                ...
                if role:
                    try:
                        ...
                        print(f"[警告] 權限不足，無法發放 {role.name} 身分組給 {interaction.user.display_name}")
            ...
            try:
                ...
                print(f"[警告] 權限不足，無法為 {interaction.user.display_name} 切換身分組")
...
class TieRackView(discord.ui.View):
    ...
    @discord.ui.button(label="將凶籤綁在神木架上", style=discord.ButtonStyle.secondary)
